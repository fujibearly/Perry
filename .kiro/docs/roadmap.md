...
1. **High-priority safety/durability:** #6 (Tool Safety Modes) and #7 (WAL Resumption) — both High, both core to the SRE stress-test. **#6a + #6b + #6c are done**, and **#6d part 1 (mTLS escalation transport + auth) is done** (with all L1 tools classified); continue with **#6d part 2** (loop integration: child escalation handler, verdict verbs, rollback journal, upward propagation, human-in-the-loop). A serious structured `_plan` (plan-driven execution + whole-plan red-light pre-pass, feeding #6c's raise-only cache) is split out as its own item (#15).
...
**Branch:** `feat/tool-safety-6d` (off `feat/tool-safety-6c`) — backlog **#6d part 1 (mTLS escalation transport + auth core)** implemented + committed; #6d part 2 (loop integration) not started.
**Version:** v0.31.0-fork.9
**Tests:** 432 pass, 0 fail (424 unit + 5 catalog-override + 3 integration) — +14 from #6d part 1 (protocol/framing/auth core + end-to-end mTLS handshake tests); +24 from #6c; +33 from #6b; +8 from #6a; +25 from #5.
**#6d transport decision (Path 1′):** mutual-TLS over a raw loopback TCP stream + hand-rolled length-delimited JSON framing — **not** WebSocket (deferred behind the `EscalationTransport` trait). Only new crate `rcgen` (+ tiny `yasna`); `rustls`/`tokio-rustls` reused (single version, no OpenSSL). Child auth = channel-bound HMAC (no literal token). Latest handoff: [`session-summary-2026-09-02-session6.md`](session-summary-2026-09-02-session6.md); continuation guide: [`handover-6d-part2.md`](handover-6d-part2.md).
**E2E demos (2026-09-04, live on `gemini-2.5-flash`):** 15 demos, whole harness on a single cheap model (`DEMO_MODEL`). Demos 1–2, 4–5, 7–8, 10–15 pass; Demo 12 offline. #6b gate demos **13 (policy forbid), 14 (authority ceiling), 15 (arg-sensitive escalation)** pass. Three residual soft-fails are model-phrasing/environment, NOT engine bugs: Demo 3 (flash formatted the plan / used `fs_patch`), Demo 6 (tmux pane-title needs an interactive controlling `/dev/tty`), Demo 9 (flash phrasing omitted the file path). Delegation demos work because delegation is not gated (decision B).
...
| 6 | Tool Safety Modes & Actuation Governance (umbrella) | 🔨 In progress — #6a+#6b+#6c done; #6d part 1 done | High | `feat/tool-safety-6a…6d` | Parallel sub-agents must never cause catastrophe; a graduated, safety-governed model gates *which* agent may perform *which* action: non-pardonable deterministic floor + LLM risk overlay + escalation to humans. | **Tenet 4 ("triage in parallel, actuate in sequence") + Pillar 5:** the missing enforcement layer for the SRE stress-test. | Umbrella; 4 stacked increments (#6a–#6d) that degrade gracefully. Spec: [`.kiro/specs/tool-safety-modes/`](../specs/tool-safety-modes/) |
...
| 6d | ↳ Escalation & control protocol + human-in-the-loop | 🔨 In progress — part 1 (transport/auth) done | High | `feat/tool-safety-6d` | mTLS inter-agent channel (child dials parent, **loopback raw-TLS + length-delimited JSON framing — not WebSocket**; WS deferred behind the `EscalationTransport` trait), typed `Escalation`/`Verdict{Halt\|Revert\|Continue}`/`Cancel`, durable rollback journal (REVERT replays it — survives connection/child death), branch-only suspension, upward propagation to orchestrator, then interactive-CLI human prompt or Layer 3. Forward-compatible with remote agents (TLS + transport-independent protocol). | **Pillar 2 (process-isolated control) + Pillar 5.** Control plane = ephemeral mTLS conn; durability plane = on-disk journal; audit plane = #14. Child auth = channel-bound HMAC (no static token). | L. **Part 1 done + committed (`d40bccc`):** `src/escalation.rs` mTLS transport + ephemeral `rcgen` cert + fingerprint-pin verifier (fail-closed) + channel-bound HMAC auth + typed protocol/framing; +14 tests incl. e2e handshake. Only new crate `rcgen`(+`yasna`). **Part 2 (loop integration) not started** — see [`handover-6d-part2.md`](handover-6d-part2.md). |
...
**Scope:** `assets/roles/%assess-risk%.md`, `src/safety.rs`, `safety.risk_model` config
