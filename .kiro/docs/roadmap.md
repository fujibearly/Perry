...
### Current State (2026-09-23 — Session 32)

**Branch:** `main` — **Project Perry (`/projects/perry`) & Innators (`/projects/innators`)**.
**Version:** v0.31.0-fork.9
**Tests:** 567 pass, 0 fail (559 unit/integration + 5 catalog-override + 3 web asset security) — suite passing in ~5.6s; clippy clean (`-D warnings`); debug and release binaries verified.
...
**Canonical Environment:** Fully migrated to `PERRY_*` as primary namespace with transparent dual exports (`set_dual_env_var`, `remove_dual_env_var`) preserving backward compatibility for child tool processes and legacy subagents.
**Actuators & Governance:** Companion harness `innators` synchronized: `web_search_perry.sh` canonical with `web_search_aichat.sh` backward-compatible symlink; `argc link-to-perry` and `argc version` native. Foundational creed codified in [`VISION.md`](VISION.md); 35 architectural anti-patterns codified in [`.kiro/docs/donts.md`](donts.md). Async test mutex starvation resolved (`tokio::sync::Mutex` for `MASK_ENV_LOCK`).
