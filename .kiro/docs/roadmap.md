...
**Branch:** `feat/tool-safety-6d` (off `feat/tool-safety-6c`) — backlog **#6d (Escalation & Control Protocol + Human-in-the-Loop)** fully implemented, verified, and hardened with **Option B Pre-flight Opportunistic Remediation** and **Supervisory Policy Enforcement & Risk Evaluation ("The Should Gate")**.
**Version:** v0.31.0-fork.9
**Tests:** 467 pass, 0 fail (459 unit + 5 catalog-override + 3 integration) — +6 from Session 10 (supervisory decision pure helper tests and escalation handler policy/ceiling tests); +9 from Session 9 (Option B preflight remediation, reversibility-aware clamp preservation, evaluator rollback context, trace enrichment); +20 from #6d; +24 from #6c; +33 from #6b; +8 from #6a; +25 from #5.
...
**E2E demos (2026-09-08, live on `gemini-2.5-flash`):** 20 demos in `scripts/run-demos.nu`. Demos 1–2, 4–5, 7–8, 10–20 pass. Includes Demo 12 (crash isolation), Demo 16 (escalation fail-closed + journal durability), Demo 17 (Happy Path autonomous write), Demo 18 (Option B Pre-flight Opportunistic Remediation on disruptive `fs_write` under `Reversible` ceiling), Demo 19 (Authority Ceiling Fail-Closed on disruptive `fs_write` under `Safe` ceiling), Demo 20 (Orchestrator to Coder Multi-Process Escalation with live supervisory %assess-risk% risk evaluation). Residual soft-fails are model-phrasing/environment (Demo 3 flash formatted the plan / used `fs_patch`, Demo 6 tmux pane-title needs interactive `/dev/tty`, Demo 9 flash phrasing omitted file path). Delegation demos work because delegation is not gated (decision B).
...
| 6c | ↳ `%assess-risk%` LLM evaluator (stricter-only overlay) | ✓ Implemented (`feat/tool-safety-6c`, unmerged) | High | `feat/tool-safety-6c` | Dedicated cheap model (`safety.risk_model`) + minimal-context `%assess-risk%` role returns a structured verdict that can only make things *stricter*. `Safe` fast-path skips it; fail-toward (low-confidence/error → block); monotonic **raise-only `RiskCache`** reuses assessments without ever green-lighting. **Enriched Context:** Inspects tool script implementation (4KB budget, UTF-8 safe, binary check, symlink resolution), semantic docstrings (`@describe`, notes, params, `@env`), and invocation preview. 100% classification across all 31 tools and agent subcommands (`83dea2e`). Absent `risk_model` → degrades to exactly #6b. | **Principle: "the LLM is not a Pardoner."** Advisory overlay clamped in Rust; assessment is an earlier red-light, never a green-light. | M — done. Role asset + `safety.rs` (verdict parse/clamp/cache/context) + `agent_loop.rs` wiring. +24 tests + context inspection tests. Literal plan-time flagging superseded by the raise-only cache (see design.md as-built); serious structured `_plan` split to its own item. |
| 6d | ↳ Escalation & control protocol + human-in-the-loop | ✓ Done (Permission vs. Authorization redesign) | High | `feat/tool-safety-permission-boundary` | Persistent per-process mTLS inter-agent channel (child dials parent, **loopback raw-TLS + length-delimited JSON framing — not WebSocket**; WS deferred behind the `EscalationTransport` trait), actor-serialized single-writer, demuxed oneshot reader, bounded drop-newest event backpressure, immediate socket-drop fail-closed, typed `Escalation`/`Verdict{Halt\|Revert\|Continue}`/`Cancel`, durable rollback journal (REVERT replays it — survives connection/child death), **Option B Pre-flight Opportunistic Remediation** (creates atomic pre-mutation journal backup on ceiling trip, stepping down required authority), **reversibility-aware verdict clamp** (preserves step-down when evaluator agrees with tool tier), evaluator `rollback_mechanism` context, **Supervisory Policy Enforcement & Risk Evaluation** ("The Should Gate": parent policy evaluation, anti-spoofed static tier and reversibility floor, supervisory %assess-risk% call with extended context, strict clamping, and human/upward routing), **Permission vs. Authorization Redesign** (hierarchical `DelegatedPermissions` upfront provisioning, no in-flight capability-mask elevation, deterministic pre-mutation unwind on `capability_denied`, structured `permission_blocked` orchestrator re-delegation with circuit-breaker cap; preserving Should Gate for authority ceiling), branch-only suspension, upward propagation to orchestrator, interactive single-key HITL CLI prompt (`[c]ontinue \| [h]alt \| [r]evert \| [e]xplain \| [g]uide`), headless Layer 3. | **Pillar 2 (process-isolated control) + Pillar 5.** Control plane = persistent mTLS conn; durability plane = on-disk 0600 journal; audit plane = #14. Child auth = channel-bound HMAC (no static token). | L — done (tasks 6d.1–6d.26). `src/escalation.rs` + `src/safety.rs` + `src/agent_loop.rs` + `src/function.rs`. mTLS listener + persistent client actor, RollbackJournal + replay, Option B preflight remediation, supervisory policy + risk evaluation, Should Gate, hierarchical permissions contract, bounded re-delegation, Demos 16–21. +40 tests. |
...
Fourth increment — persistent mTLS control plane, human-in-the-loop, durable rollback journaling, and Option B opportunistic remediation:
...
- +29 unit/integration tests (461 total workspace tests); Demo 16 offline fail-closed and journal durability verified.
