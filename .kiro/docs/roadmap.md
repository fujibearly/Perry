...
**Branch:** `feat/tool-safety-6d` (off `feat/tool-safety-6c`) — backlog **#6d (Escalation & Control Protocol + Human-in-the-Loop)** fully implemented and verified.
**Version:** v0.31.0-fork.9
**Tests:** 446 pass, 0 fail (438 unit + 5 catalog-override + 3 integration) — +20 from #6d (protocol/framing/auth core, persistent per-process client multiplexing, cancellation propagation, event backpressure, loop escalation integration, rollback journal durability, fail-closed handling); +24 from #6c; +33 from #6b; +8 from #6a; +25 from #5.
