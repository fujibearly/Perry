...
1. **High-priority safety/durability:** #6 (Tool Safety Modes) and #7 (WAL Resumption) — both High, both core to the SRE stress-test. **#6a + #6b + #6c are done** (with all L1 tools classified); continue with **#6d** (mTLS-WebSocket escalation channel + human-in-the-loop). A serious structured `_plan` (plan-driven execution + whole-plan red-light pre-pass, feeding #6c's raise-only cache) is split out as its own item (#15).
...
**Branch:** `feat/tool-safety-6c` (off `feat/tool-safety-6b`) — backlog #6c implemented (LLM risk evaluator overlay)
**Version:** v0.31.0-fork.9
**Tests:** 418 pass, 0 fail (410 unit + 5 catalog-override + 3 integration) — +24 from #6c (verdict parse/minimal-context/stricter-only clamp/raise-only cache + evaluator wiring); +33 from #6b; +8 from #6a; +25 from #5.
...
| 6 | Tool Safety Modes & Actuation Governance (umbrella) | 🔨 In progress — #6a+#6b+#6c done | High | `feat/tool-safety-6a…6d` | Parallel sub-agents must never cause catastrophe; a graduated, safety-governed model gates *which* agent may perform *which* action: non-pardonable deterministic floor + LLM risk overlay + escalation to humans. | **Tenet 4 ("triage in parallel, actuate in sequence") + Pillar 5:** the missing enforcement layer for the SRE stress-test. | Umbrella; 4 stacked increments (#6a–#6d) that degrade gracefully. Spec: [`.kiro/specs/tool-safety-modes/`](../specs/tool-safety-modes/) |
