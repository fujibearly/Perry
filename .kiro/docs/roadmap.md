...
1. **High-priority safety/durability:** #6 (Tool Safety Modes) and #7 (WAL Resumption) — both High, both core to the SRE stress-test. **#6a + #6b are done** (with all L1 tools classified); continue with **#6c** (LLM risk evaluator) → **#6d** (mTLS-WebSocket escalation channel + human-in-the-loop).
2. **#12 Remote MCP** when L4 integration becomes concrete (the bridge that makes any L4 control plane useful). #6d's WebSocket channel and #12 share the same WSS/transport muscle — the remote-agent generalization of #6d rides on it.
...
**Branch:** `feat/tool-safety-6b` (off `feat/tool-safety-6a`) — backlog #6b implemented + follow-ups
**Version:** v0.31.0-fork.9
**Tests:** 393 pass, 0 fail (385 unit + 5 catalog-override + 3 integration) — +33 from #6b (tiers/reversibility/policy/ceiling/clamp + ToolBlocked trace); +8 from #6a; +25 from #5.
...
| 6 | Tool Safety Modes & Actuation Governance (umbrella) | 🔨 In progress — #6a+#6b done | High | `feat/tool-safety-6a…6d` | Parallel sub-agents must never cause catastrophe; a graduated, safety-governed model gates *which* agent may perform *which* action: non-pardonable deterministic floor + LLM risk overlay + escalation to humans. | **Tenet 4 ("triage in parallel, actuate in sequence") + Pillar 5:** the missing enforcement layer for the SRE stress-test. | Umbrella; 4 stacked increments (#6a–#6d) that degrade gracefully. Spec: [`.kiro/specs/tool-safety-modes/`](../specs/tool-safety-modes/) |
