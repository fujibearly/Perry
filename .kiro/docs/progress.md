...
| 6a | ↳ Deterministic capability mask (floor / fallback) | In progress (`feat/tool-safety-6a`) | High | `feat/tool-safety-6a` | Binary `readonly`/`mutating` mask; sub-agents read-only by default; **unclassified tools reserved to humans**. The permanent floor everything degrades to. | **Pillar 2 + Pillar 5:** capability mask rides the child-PID env channel (like `AICHAT_AGENT_DEPTH`). | S–M — `function.rs` (`ToolMode` field), `agent_loop.rs` (mask propagation + gate) |
