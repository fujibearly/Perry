...
**Tests:** 348 tests pass, 0 fail (340 unit + 5 catalog-override + 3 integration) — +21 from backlog #5 hardening  
...
| 5 | Test Suite & Coverage Hardening | In Progress | Medium | `feat/test-suite-hardening` | Coverage analysis showed strong baseline (72.9% line) but untested edge paths in error handling, crash isolation, cyclic pipe aborts, and budget conditions. | **Pillar 5 (Deterministic Safety):** validates the circuit-breaker/budget guarantees the fork claims. Cross-cutting; hardens existing behavior rather than adding capability. | M — ~200-400 lines of tests; low risk, mechanical. Landed: +21 unit tests (17 agent_loop + 4 mcp) + Demo 12 (offline sub-agent crash isolation) in `scripts/run-demos.nu`. Coverage: agent_loop.rs 46.8%→62.5% line. FR-4 closed via Demo 12. |
