...
- [x] 4.1 (stretch) `subagent_nonzero_exit_yields_agent_error` — **COVERED via E2E, not a unit test.** Not hermetically achievable at the unit level: `eval_agent_tool_subprocess` spawns `std::env::current_exe()`, which under `cargo test` is the *test* binary, not the `aichat` binary. Instead, added **Demo 12 (Sub-Agent Crash Isolation)** to `scripts/run-demos.nu` — a deterministic, offline check that runs the real binary with an unknown agent name and asserts the failing child exits non-zero, emits a captured/readable error (`Unknown agent ...`), and does not panic. This exercises the exact non-zero-exit → `agent_error` signal `eval_agent_tool_subprocess` relies on. The depth-guard boundary (FR-3.1) remains covered by the `agent_depth_guard_rejects_at_exact_max` unit test. (FR-4.1)
