...
- [x] 6d.20 Delegation permissions contract in `src/function.rs` and `src/config/agent.rs`: define `DelegatedPermissions`, `validate_against_parent`, safe-floor clamping, flat argument fallbacks, and schema exposure. (FR-6d.18)
- [x] 6d.21 Subprocess spawning & env provisioning in `src/agent_loop.rs`: parse `permissions` (or flat fallback) in `eval_agent_tool_subprocess`, validate against parent ceiling & mask, set `AICHAT_CAPABILITY_MASK` and `AICHAT_AUTHORITY_CEILING`. (FR-6d.18)
- [x] 6d.22 Child capability gate immediate denial & unwinding in `src/agent_loop.rs`: update `eval_single_tool` so `capability_denied` does not escalate over mTLS; in child loop, unwind journal entries, emit trace, and exit with structured `permission_blocked` payload. Retain mTLS escalation for `authority_exceeded`. (FR-6d.19)
- [x] 6d.23 Orchestrator loop ingestion & bounded re-delegation in `src/agent_loop.rs`: parse `permission_blocked` tool result, format re-delegation guidance, and enforce re-delegation attempt cap. (FR-6d.20)
...
- [x] 6d.33 Hierarchical Guide Rails, Semantic Role Badging & Turn Delta Observability: Update `format_dialog_block` to render colored vertical guide rails based on recursion depth and agent palette, with asymmetric `📥 PROMPT` vs `📤 RESPONSE` framing; update `format_messages_dialog` with semantic role coloring, system prompt dimming/folding on turn > 1, and `⚡ [new]` delta highlighting vs `[history]` dimming. (FR-6d.25)
- [x] 6d.34 Verification, Testing & Demo Observability Validation: Add/update unit tests for dialog formatting, guide rails, and message delta highlighting; run full test suite, clippy, and live demos (Demo 3, 4, 5) with `--dialog --no-truncate` to verify visual readability and absence of repetition illusion. (Verification)
- [x] 6d.35 ANSI-Aware Soft-Wrapping, Guide Rail Continuity & Responsive Box Framing: Implement `strip_ansi`, `visible_width`, `get_terminal_width`, and `wrap_ansi_line` to dynamically wrap long lines at word boundaries without cutting escape codes or exceeding terminal width; ensure all wrapped continuation lines prepend vertical guide rails with context-appropriate hanging indents (code/JSON/badges/lists); responsively clamp dialog header/footer borders to terminal width; maintain ancestor guide rails through progress trace lines. (FR-6d.25)
- [x] 6d.36 Verification & Validation of Soft-Wrapping and Rails Integrity: Add unit tests for ANSI stripping, visible width, soft-wrapping, and guide rail preservation; verify full test suite (494 passed), clippy clean, release build, and live Demos 3 and 4 with `--dialog --no-truncate`. (Verification)
- [x] 6d.37 Explicit Blocked Comparisons, Debug Journal Inspection & Deterministic Petnames: Add explicit mathematical comparisons (`risk > ceiling`) backing blocked decisions in `authority_denied_result`, `--debug` CLI flag with guide-rail rollback journal metadata inspection omitting backup contents, deterministic petname generation (`format_agent_pid(pid)`), and bounded helper script resolution (`utils/guard_path.sh`). (FR-6d.26)
- [x] 6d.38 Unified ALLOW / BLOCK Governance Nomenclature & Single-Source Derivation: Standardize trace event formatting to `<VERB> <tool>: <lhs> <op> <rhs>` with `risk` always on LHS and `ceiling` on RHS; implement pure `format_risk_token(static_tier, required, mechanism)` for parenthetical `(effective, <why>)` qualifiers; thread `ceiling` into `prompt_human_verdict` and update `[HUMAN APPROVAL REQUIRED]` banner; update unit tests, demos, and architecture documentation. (FR-6d.27)
- [x] 6d.39 Documentation & Session Summary 18: Update `roadmap.md`, `architecture.md`, `README.md`, `CHANGELOG.md`, and record Session Summary 18. (Verification)
