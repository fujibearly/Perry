...
- **FR-6d.25 — Hierarchical Guide Rails, Semantic Role Badging & Turn Delta Observability.**
  1. **Color-Coded Vertical Guide Rails:** Dialog blocks rendered via `format_dialog_block` MUST visually display depth hierarchy using colored vertical guide rails corresponding to the agent's distinct ANSI palette (`orchestrator`: purple, `coder`: green, `researcher`: yellow, etc.), so sub-agent execution context is immediately anchored during scrolling.
  2. **Asymmetric Request vs. Response Framing:** Prompts submitted to the LLM (`DialogDirection::Request`) and responses received (`DialogDirection::Response`) MUST have distinct header styling, iconography (`📥 PROMPT` vs `📤 RESPONSE`), and colored framing to prevent visual confusion between inputs and generations.
  3. **Semantic Role Badging & Boilerplate Dimming:** In `format_messages_dialog`, role headers MUST be styled semantically (`[user]` in bold cyan, `[assistant]` in bold yellow, `tool_calls:` in bold light blue, `tool_result: <tool>` in bold magenta). Static boilerplate instructions (`[system]`) MUST be visually dimmed.
