...
2. **Local Artifact Hyperlinks**:
   - Surface all files in the JSON `artifacts` object as clickable markdown links with clean absolute paths without the `file://` prefix (e.g., `[Log Dump](/tmp/perry-host_logs-dump-....log)` and `[Log Query](/tmp/perry-host_logs-query-....log)`).
   - **Artifact Safety Rule**: When surfacing local artifacts, scripts (such as `.sh`, `.bash`, `.py`, `.nu`) must NEVER be linked directly with executable extensions. Surface them as `.log` files (e.g. `query.log` or `query.sh.log`) to prevent accidental shell execution when following URLs.
